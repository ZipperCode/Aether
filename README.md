...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `SEARCH_POOL_GATEWAY_DB_PATH` | `/sqlites/search_pool_gateway.db` | 搜索池网关独立 SQLite 路径 |
| `SEARCH_POOL_GATEWAY_CRYPTO_KEY` | 空 | 搜索池网关 API Key 加密密钥，建议显式配置 |
...
#    将 docker-compose.yml 中 image 从 ghcr.io/zippercode/aether:latest 改为指定版本
#    方式二：使用之前记录的镜像 digest
#    将 image 改为 ghcr.io/zippercode/aether@sha256:xxxxx
docker compose up -d app
```

> 可以在升级前通过 `docker inspect ghcr.io/zippercode/aether:latest --format '{{index .RepoDigests 0}}'` 记录当前镜像 digest，方便回滚时使用。
...
[![Star History Chart](https://api.star-history.com/svg?repos=ZipperCode/Aether&type=Date)](https://star-history.com/#ZipperCode/Aether&Date)
