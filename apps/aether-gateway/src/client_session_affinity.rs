...
/// 从一次 Codex 请求提取的会话、turn 与缓存信号；各字段按独立优先级解析。
pub(crate) struct CodexRequestSignals {
    /// 用于调度亲和的根会话标识。
    pub(crate) session_id: Option<String>,
    /// 原生 thread 标识；存在时优先于会话派生值。
    pub(crate) thread_id: Option<String>,
    /// 客户端逻辑 turn 标识，不得用普通 request-id 代替。
    pub(crate) turn_id: Option<String>,
    /// 客户端显式缓存键；仅在最终正文仍保留时参与传输收敛。
    pub(crate) prompt_cache_key: Option<String>,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
/// `x-codex-turn-metadata` 内部可识别的最小身份字段集合。
...
/// 依次使用显式 Aether、已识别客户端和通用适配器解析一个会话范围。
...
/// 从 HTTP 头和可选正文统一提取 Codex 信号，供亲和与指纹上下文共享同一解析规则。
...
impl ClientSessionScopeAdapter for CodexSessionScopeAdapter {
    ...
    /// 使用与指纹上下文相同的 Codex 会话优先级，避免调度亲和与传输身份分叉。
    fn extract_scope(&self, request: &ClientSessionRequest<'_>) -> Option<ClientSessionScope> {
...
/// 从 Aether 自有会话头建立最高优先级范围，并保留可选 agent 隔离维度。
...
/// 按字段独立优先级解析 Codex 请求，避免 session、thread、turn 相互错误覆盖。
...
/// 从请求构建 Codex 会话范围，并复用已经解析的嵌套 turn 元数据。
...
/// 按 Aether 显式头、Codex 原生头、正文元数据和通用回退解析会话范围。
...
/// 构造 Codex 会话范围，并附加账号提示以防跨账号亲和碰撞。
...
/// 同时接受对象或 JSON 字符串形态的嵌套 turn 元数据，其他类型返回空信号。
...
/// 解析头部中的 JSON turn 元数据；畸形内容安全回退为空信号。
...
/// 从 turn 元数据对象读取三个已知字符串字段，不接纳其他字段作为身份。
...
/// 不区分大小写判断是否存在指定前缀的头部，用于识别客户端家族而不读取敏感值。
...
mod tests {
    ...
    /// 验证 Codex 原生 session 头可生成预期会话范围。
    #[test]
    ...
    /// 验证正文会话信号优先于无业务语义的请求 ID，避免每请求破坏亲和。
    #[test]
    ...
    /// 验证 Codex 会话信号按显式 Aether 头、原生头、正文和通用字段依次回退。
    #[test]
    ...
    /// 验证 thread 与缓存键独立提取，不因 session 来源不同而相互覆盖。
    #[test]
    ...
    /// 验证 Codex Live 的 `x-session-id` 可作为会话范围来源。
    #[test]
    ...
    /// 验证 Responses 原生 session 头优先于 Live 的兼容会话头。
    #[test]
    ...
    /// 验证 turn 标识按正文元数据、嵌套元数据和头部元数据的既定顺序解析。
    #[test]
