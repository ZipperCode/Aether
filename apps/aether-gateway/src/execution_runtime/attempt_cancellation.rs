...
use crate::execution_runtime::transport_failure::{
...
impl AttemptCancellationGuard {
    ...
    pub(crate) async fn fail_and_disarm(&mut self, error: &GatewayError) {
        ...
        let (status_code, error_type, error_message) = match error {
            ...
            _ => (
                ...
                "Local stream attempt failed before terminal finalization".to_string(),
            ),
        };
        mark_stream_candidate_watchdog_terminal_started();
        let state = self.state.clone();
        let request_id = armed.request_id.clone();
        // 所有权先移入独立任务；等待者取消也不会截断候选与 usage 的终态写入。
        ...
        if let Err(error) = terminal_write.await {
            warn!(
                event_name = "local_attempt_terminal_write_join_failed",
                log_type = "ops",
                request_id = %short_request_id(&request_id),
                error = %error,
                "gateway failed to join the isolated attempt terminal write"
            );
        }
    }
}

/// 使用 Pending 时的紧凑身份和正文捕获描述写入唯一终态，不估算缺失的 token 使用量。
async fn settle_attempt(
    ...
    usage_data.error_category = Some(
        match usage_event_type {
            ...
            _ if status_code >= 500 => "server_error",
            _ if status_code >= 400 => "client_error",
    ...
    state
        ...
        .record_terminal_event_direct(
            ...
            UsageEvent::new(usage_event_type, request_id, usage_data),
        )
        .await;
}

impl Drop for AttemptCancellationGuard {
    /// 只有仍持有 Pending 且 watchdog 未接管的取消路径才派发终态写入。
    fn drop(&mut self) {
        let Some(armed) = self.armed.take() else {
            return;
        };
