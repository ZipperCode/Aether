...
/// 以唯一取消 Guard 包装流执行，正常返回后把生命周期交给响应体 finalizer。
pub(crate) fn execute_execution_runtime_stream<'a>(
    ...
    Box::pin(async move {
        ...
        .await;
        // 错误仍由同一个 Guard 结算；成功返回后才把终态所有权交给响应体。
        if let Err(error) = result.as_ref() {
            cancellation_guard.fail_and_disarm(error).await;
        } else {
...
#[allow(clippy::too_many_arguments)]
/// 创建带重试范围回传的流式候选 Future，供调度器按候选失败继续尝试。
...
> {
    Box::pin(async move {
        ...
        .await;
        // 错误与取消共用唯一终态所有者，避免准入错误被错误记录成取消或 500。
        if let Err(error) = result.as_ref() {
            cancellation_guard.fail_and_disarm(error).await;
        } else {
            cancellation_guard.disarm();
...
/// 仅在本地故障转移策略明确停止时，把流式传输错误转换为最终响应。
...
/// 执行单个流式候选，并在写入 Pending 前原子获取 Provider Key 与 Pool 在途许可。
/// 并发饱和只跳过当前候选；取得的许可会传给后续流生命周期统一释放。
#[allow(clippy::too_many_arguments)]
...
mod tests {
    ...
    use super::{
        ...
        wrap_non_json_binary_stream_error_for_client, AttemptCancellationGuard,
    ...
    /// 验证完整正文记录下唯一终态 Guard 对 admission timeout 只结算一次，并保持 429/void 语义。
    #[tokio::test]
    async fn stream_attempt_guard_records_admission_timeout_once_as_429() {
        ...
        let state = AppState::new()
            ...
            .with_data_state_for_tests(
                ...
                // 默认 basic 不保留正文；本回归显式开启 full 以核对客户端错误类型。
        ...
        let mut guard = AttemptCancellationGuard::disarmed(
            &state,
            super::STREAM_ATTEMPT_CANCELLED_ERROR_TYPE,
            super::STREAM_ATTEMPT_CANCELLED_ERROR_MESSAGE,
        );
        guard.arm(
            &plan,
            Some(&report_context),
    ...
    /// 构造 Responses 失败流；可选择现场裸错误，或控制标准终端错误前是否已有客户端可见事件。
    ...
    async fn prefetched_codex_cyber_policy_violation_stops_failover_by_default() {
        let response = execute_prefetched_codex_cyber_policy_failure(false, true, false)
            .await
    ...
    async fn prefetched_codex_cyber_policy_violation_retries_when_routing_strategy_is_enabled() {
        assert!(
            execute_prefetched_codex_cyber_policy_failure(true, false, false)
                .await
                .is_none(),
    ...
    async fn same_format_responses_prefetch_retries_bare_error_before_committing_success() {
        assert!(
            execute_prefetched_codex_cyber_policy_failure(true, false, true)
                .await
                .is_none(),
...
"#,
    ...
    /// 验证真实 Key catalog 下的重定向保持上游状态与审计头，但对客户端返回结构化失败。
    #[tokio::test]
    ...
    /// 验证已认证 tunnel 与强 Key catalog 共同参与请求，首帧前错误按上游规则返回受限诊断。
    #[tokio::test]
    ...
    /// 验证已认证 tunnel 发出可见语义内容后，故障留在同一 SSE 流并写入终态错误。
    #[tokio::test]
