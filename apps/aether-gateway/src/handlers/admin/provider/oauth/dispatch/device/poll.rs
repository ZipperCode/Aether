...
pub(super) async fn handle_admin_provider_oauth_device_poll(
    ...
    let operation = async {
        ...
        // Nous 使用自身设备授权流程，并共享上游轮询租约及终态持久化边界。
        if provider_type == "nous" {
                ...
                .await
            {
                ...
                Err(error) => {
                    ...
                    if detail.contains("authorization_pending") {
                        return Ok(
                            Json(json!({"status":"pending","replaced":false})).into_response()
                        );
                    }
                    if detail.contains("slow_down") {
                        return Ok(
                            Json(json!({"status":"slow_down","replaced":false})).into_response()
                        );
                    }
                ...
                .await
            {
                ...
                Err(detail) => {
                    return Ok(
                        Json(json!({"status":"error","error":detail,"replaced":false}))
                            .into_response(),
                    )
                }
            };
