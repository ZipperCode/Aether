...
impl<'a> AdminAppState<'a> {
    ...
    /// 单独导出用户时排除管理员，凭据投影仍由统一导出模式决定。
    pub(crate) async fn build_admin_system_users_export_payload(
        &self,
        mode: SystemExportMode,
    ) -> Result<serde_json::Value, GatewayError> {
        self.build_admin_system_users_export_payload_with_admin_scope(false, mode)
            .await
    }

    /// 用户范围与敏感字段模式独立：完整备份包含管理员，但不绕过凭据导出策略。
    ...
    pub(crate) async fn build_admin_system_data_export_payload(
        ...
        let user_data = self
            .build_admin_system_users_export_payload_with_admin_scope(true, mode)
            .await?;
