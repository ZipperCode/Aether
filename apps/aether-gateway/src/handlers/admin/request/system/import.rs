...
use aether_data_contracts::repository::global_models::{
    CreateAdminGlobalModelRecord, UpdateAdminGlobalModelRecord,
...
/// 还原导入模式允许保留的模型 JSON 字段；显式 Endpoint 绑定随后独立重映射并原子写入。
...
/// 旧版本没有独立绑定表时，从明确格式证据解析旧 Endpoint 标识；多 Endpoint 歧义报错。
...
/// 将备份内的 Endpoint 标识替换为目标环境标识，保留已有旧格式映射语义。
...
/// 通过明确 API 格式将旧版备份 Endpoint 引用映射到当前 Provider 的本地标识。
...
/// 构造绑定记录并保留来源和启用状态；跨 Provider 引用、重复绑定及显式空列表报错。
...
/// 预校验一个 Provider 的全部绑定；在导入开始写入前拒绝未知 Endpoint 或旧格式歧义。
...
impl<'a> AdminAppState<'a> {
    /// 旧配置沿用已有模型推断；没有显式证据时保留旧版 Provider 范围迁移语义。
    ...
    /// 在任何持久化前校验完整导入文档、模式策略及本地 Endpoint 绑定证据。
    async fn prevalidate_admin_system_config_import(
        ...
        for imported_provider_item in &imported_providers {
            ...
            invalid!(validate_import_provider_model_endpoint_bindings(
                ...
                source_version != ADMIN_SYSTEM_CONFIG_EXPORT_VERSION,
            ));
    ...
    /// 按统一模式预校验后导入；保留凭据还原和回滚日志，并原子创建或覆盖模型及精确绑定。
...
mod tests {
    ...
    #[test]
    /// 2.4 绑定往返保留来源和禁用状态，同时采用上游敏感 JSON 投影而不丢失目标映射。
    fn config_import_preserves_endpoint_bindings_with_secret_safe_model_projection() {
        let provider: super::ImportedProvider = serde_json::from_value(json!({
            "name": "provider",
            "endpoints": [{
                "id": "exported-chat", "api_format": "openai:chat",
                "base_url": "https://example.com", "is_active": true
            }],
            "models": [{
                "global_model_name": "model", "provider_model_name": "upstream-model",
                "provider_model_mappings": [{"endpoint_ids": ["exported-chat"]}],
                "endpoint_bindings": [{
                    "endpoint_id": "exported-chat", "source": "manual", "is_active": false
                }],
                "config": {"credentials": "***", "region": "eu-west-1"}
            }]
        }))
        .unwrap();
        super::validate_import_provider_model_endpoint_bindings(&provider, "custom", false)
            .unwrap();
        let exported = serde_json::to_value(&provider).unwrap();
        assert_eq!(exported["endpoints"][0]["id"], "exported-chat");
        assert_eq!(
            exported["models"][0]["endpoint_bindings"][0]["is_active"],
            false
        );
        let model = &provider.models[0];
        let id_map = std::collections::BTreeMap::from([(
            "exported-chat".to_string(),
            "local-chat".to_string(),
        )]);
        let record = super::build_import_provider_model_record(
            "provider",
            None,
            None,
            "global-model",
        ...
        .unwrap();
        let record = super::remap_import_provider_model_endpoint_ids(record, &id_map, None);
        assert_eq!(
            record.provider_model_mappings.as_ref().unwrap()[0]["endpoint_ids"],
            json!(["local-chat"])
        );
        assert!(record.config.as_ref().unwrap().get("credentials").is_none());
        assert_eq!(record.config.as_ref().unwrap()["region"], "eu-west-1");
        let bindings =
            super::build_import_provider_model_endpoint_bindings(&record.id, model, &id_map)
                .unwrap()
                .unwrap();
        assert_eq!(bindings[0].endpoint_id, "local-chat");
        assert_eq!(bindings[0].source, "manual");
        assert!(!bindings[0].is_active);
        let mut foreign = provider.clone();
        foreign.models[0].endpoint_bindings.as_mut().unwrap()[0].endpoint_id =
            "foreign".to_string();
        assert!(
            super::validate_import_provider_model_endpoint_bindings(&foreign, "custom", false)
                .is_err()
        );
    }
