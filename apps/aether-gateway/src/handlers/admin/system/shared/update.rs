...
/// 校验 tarball 与校验和清单均属于本 fork 的同版本、当前平台发布资产。
fn validate_update_release_urls(
    ...
    // 版本与制品必须同时属于本 fork；沿用上游同版本 tarball/checksum 绑定校验。
    let release_prefix = format!("/ZipperCode/Aether/releases/download/{safe_version}/");
...
mod tests {
    ...
    #[test]
    /// 保留上游 HTTPS/下载域约束，并接受 GitHub 的 release-assets 重定向域。
    fn update_validates_download_urls() {
        assert!(validate_update_download_url(
            "https://github.com/ZipperCode/Aether/releases/download/v1/aether.tar.gz"
        )
        .is_ok());
        assert!(validate_update_download_url(
        ...
        .is_ok());
        assert!(validate_update_download_url(
            "http://github.com/ZipperCode/Aether/releases/download/v1/aether.tar.gz"
        )
        .is_err());
        assert!(validate_update_download_url(
            "https://user@github.com/ZipperCode/Aether/releases/download/v1/aether.tar.gz"
        )
        .is_err());
        assert!(
    ...
    #[test]
    /// 本 fork 的同版本资产通过，跨仓库、错版本和错架构组合必须失败。
    fn update_binds_tarball_and_checksum_to_official_same_version_release() {
        ...
        let tarball = format!(
            "https://github.com/ZipperCode/Aether/releases/download/v1.2.3/aether-v1.2.3-{platform}-{arch}.tar.gz"
        );
        let checksums = "https://github.com/ZipperCode/Aether/releases/download/v1.2.3/SHA256SUMS";
        ...
        for (bad_tarball, bad_checksums) in [
            (
                tarball.replace("ZipperCode/Aether", "attacker/project"),
                checksums.to_string(),
            ),
