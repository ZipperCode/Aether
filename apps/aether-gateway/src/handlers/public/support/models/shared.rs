...
pub(super) fn models_detail_id(request_path: &str) -> Option<String> {
    let raw = request_path
        ...
        .or_else(|| request_path.strip_prefix("/v1beta/models/"))?;
...
fn known_format_family(api_format: &str) -> Option<ModelFamily> {
...
fn capability_family(value: &str) -> Option<ModelFamily> {
    match value.trim().to_ascii_lowercase().as_str() {
        "generation" | "text_generation" | "chat" => Some(ModelFamily::Generation),
        "image" | "image_generation" => Some(ModelFamily::Image),
