...
mod tests {
    ...
    /// 构造 Codex OAuth 持久化测试状态，保留既有测试调用的专用入口。
    ...
    /// 按 Provider 类型构造带加密 access/auth 配置的 OAuth 测试目录与 AppState。
    ...
    /// 验证 Antigravity 刷新结果会规范化并持久化 access token、refresh token 与过期时间。
    #[tokio::test]
