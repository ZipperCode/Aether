...
#[test]
/// 验证全新目录导入将导出 Endpoint ID 映射到新实例 ID。
...
#[test]
/// 验证旧版导入只在格式证据唯一时重映射 Endpoint ID。
...
#[test]
/// 验证含歧义的旧 Endpoint ID 导入被拒绝且存储保持不变。
...
#[test]
/// 验证旧模型导入按既有迁移契约绑定 Provider 的全部 Endpoint。
...
#[test]
/// 验证空 Endpoint 绑定在写入前被拒绝，不覆盖既有模型。
...
#[test]
/// 验证等价格式引起的 Endpoint 绑定冲突不会产生部分导入。
...
#[test]
/// 验证固定 Provider 不支持的 Endpoint 绑定在写入前被拒绝。
