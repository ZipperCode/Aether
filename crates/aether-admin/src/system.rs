...
/// 配置备份 2.4 保留显式模型 Endpoint 绑定，兼容已有 2.0–2.3 导入。
pub const ADMIN_SYSTEM_CONFIG_EXPORT_VERSION: &str = "2.4";
/// 可导入的配置格式版本；2.4 以前使用既有 Endpoint 推断规则。
...
/// 用户备份采用上游 1.6 策略字段，不与配置备份版本共用编号。
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
/// 配置备份内的模型 Endpoint 精确绑定；导入时 Endpoint 标识按 Provider 重映射。
pub struct AdminSystemConfigModelEndpointBinding {
    /// 同一 Provider 导出 Endpoint 的原始标识，不直接复用为目标数据库主键。
    pub endpoint_id: String,
    /// 绑定来源，如 manual、mapping 或 import；往返保存时不得丢失。
    pub source: String,
    /// 此绑定是否启用；缺省为 true，显式 false 必须保留。
    #[serde(default = "default_true")]
    pub is_active: bool,
}
...
pub struct AdminSystemConfigProviderModel {
    ...
    /// 独立精确绑定列表；缺省使用既有推断，显式列表在导入预校验中检查。
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// 返回系统设置默认值；本地 Agent bridge 与上游 failover/凭据策略在同一配置表中生效。
...
/// 统一规范化系统设置更新，保留 Agent bridge 枚举与上游敏感字段类型校验。
