...
/// 将规范请求投影为 Gemini 请求；跨格式混合工具仅在 Gemini 3 上启用服务端调用标记。
pub fn to_raw(
...
/// 判断请求是否携带同格式 Gemini 原始工具；原始工具由扩展负责还原，不重复规范化。
...
/// 为跨格式混合工具执行模型门禁；不支持该能力的目标返回 None 使转换失败。
...
/// 在已确定兼容的 Gemini 请求上写入 camelCase 混合工具开关，并移除蛇形别名。
...
/// 通过唯一 Gemini 工具投影判断规范请求是否同时含内置工具与函数声明。
...
/// 判断 Gemini wire 工具数组是否同时包含非空函数声明和至少一个内置工具。
...
/// 合并相邻同角色消息并生成 Gemini contents；用户侧工具结果会按前一批调用顺序对齐。
...
/// 仅对明确来自跨格式适配器且能完整一一配对的工具结果排序；不确定时保持原顺序。
...
/// 将一组规范内容块转为 Gemini parts，并只给首个无真实签名的合成工具调用添加哨兵。
...
/// 转换单个规范内容块；`is_first_tool_use` 仅控制 Gemini 合成历史的签名校验哨兵。
...
/// 复制 Gemini 支持的 Schema 字段，归一联合类型、nullable、枚举和嵌套对象。
...
/// 将 Gemini 以字符串承载的 int64 约束从数字或字符串规范化为字符串。
...
/// 仅接受全字符串数组，避免把非法 required/propertyOrdering 静默发给 Gemini。
...
mod tests {
    ...
    /// 验证复杂 JSON Schema 被解析并裁剪为 Gemini 支持的稳定结构。
    #[test]
    ...
    /// 验证工具结果转换仍保留与函数调用配对所需的 response ID。
    #[test]
    ...
    /// 验证混合工具仅允许 Gemini 3，Gemini 2.5 在传输前失败。
    #[test]
