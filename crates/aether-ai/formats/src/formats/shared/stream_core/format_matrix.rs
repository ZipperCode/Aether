...
impl StreamingStandardFormatMatrix {
    ...
    /// 顺序发射规范帧；完整终态错误对 Responses/Chat 保真，其余客户端转换为本地错误格式。
    fn emit_frames(
        &mut self,
        report_context: &Value,
...
impl StreamingStandardTerminalObserver {
    ...
    /// 将规范流帧合并进终态摘要，并从完整失败事件提取错误与 usage。
...
impl ClientStreamEmitter {
    ...
    /// 保留 Responses/Chat 可表达的完整失败帧；其他格式仅转换其结构化错误主体。
...
mod tests {
    ...
    /// 验证 Gemini 首个非空 thought 立即转换为 Responses reasoning delta。
    #[test]
    ...
    /// 验证已提交 Gemini 流的畸形工具终止原因输出完整 Responses 失败事件。
    #[test]
