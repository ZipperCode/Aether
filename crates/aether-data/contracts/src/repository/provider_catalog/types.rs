...
/// 凭证及旧调度快照双重 CAS，只更新 `status_snapshot.scheduling`，保留其他状态。
/// `None` 持久化为 JSON null，使 memory 与 PostgreSQL 的清空语义一致。
#[derive(Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct ProviderCatalogKeySchedulingStateCasUpdate {
    /// 待更新的 Provider Key 标识。
    pub key_id: String,
    /// 观察到的 API Key 密文；替换凭证后旧响应不得更新调度状态。
    pub expected_encrypted_api_key: Option<String>,
    /// 观察到的认证配置密文，包含 SQL NULL 的精确语义。
    pub expected_encrypted_auth_config: Option<String>,
    /// 观察到的认证类型；比较时忽略大小写。
    pub expected_auth_type: String,
    /// 写入前应当存在的调度对象；None 同时代表缺失及 JSON null。
    pub expected_scheduling: Option<serde_json::Value>,
    /// 新调度对象；None 仅清空调度命名空间，不影响其他状态字段。
    pub scheduling: Option<serde_json::Value>,
    /// 最后更新时间，Unix 秒；缺失时仓储使用当前时间。
    pub updated_at_unix_secs: Option<u64>,
}

impl std::fmt::Debug for ProviderCatalogKeySchedulingStateCasUpdate {
    /// 沿用上游 CAS 的调试约束，不把凭证密文及调度证据输出到 Debug。
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ProviderCatalogKeySchedulingStateCasUpdate")
            .field("key_id", &self.key_id)
            .field("expected_auth_type", &self.expected_auth_type)
            .field("updated_at_unix_secs", &self.updated_at_unix_secs)
...
mod transport_tests {
    use super::{
        ...
        ProviderCatalogKeyOAuthRuntimeStateCasUpdate, ProviderCatalogKeySchedulingStateCasUpdate,
    ...
    fn provider_catalog_cas_debug_output_redacts_credential_fences() {
        let scheduling = ProviderCatalogKeySchedulingStateCasUpdate {
            ...
            expected_encrypted_api_key: Some("scheduling-api-key-canary".to_string()),
            expected_encrypted_auth_config: Some("scheduling-auth-config-canary".to_string()),
            expected_auth_type: "oauth".to_string(),
            expected_scheduling: Some(serde_json::json!({"reason": "scheduling-old-canary"})),
