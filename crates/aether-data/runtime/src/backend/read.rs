...
impl DataReadRepositories {
    ...
    fn install_postgres(&mut self, backend: &PostgresBackend) {
        ...
        if self.model_catalog.is_none() {
            self.model_catalog = Some(PostgresBackend::model_catalog_read_repository(backend));
        }
    ...
    fn install_mysql(&mut self, backend: &MysqlBackend) {
        ...
        if self.model_catalog.is_none() {
            self.model_catalog = Some(MysqlBackend::model_catalog_read_repository(backend));
        }
        if self.oauth_providers.is_none() {
    ...
    fn install_sqlite(&mut self, backend: &SqliteBackend) {
        ...
        if self.model_catalog.is_none() {
            self.model_catalog = Some(SqliteBackend::model_catalog_read_repository(backend));
        }
        if self.oauth_providers.is_none() {
