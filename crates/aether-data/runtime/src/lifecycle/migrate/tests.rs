...
struct ManagedPostgresServer {
    ...
    workdir: PathBuf,
    database_url: String,
    /// Admin URL used to create and drop an isolated database on a shared server.
    admin_database_url: Option<String>,
    /// Database created for this fixture; dropped during cleanup when present.
    owned_database_name: Option<String>,
}

impl ManagedPostgresServer {
    async fn try_start() -> Result<Option<Self>, Box<dyn std::error::Error>> {
        // Prefer a shared external Postgres, such as the CI service container. Each fixture
        // still receives an isolated database so sequential tests cannot affect one another.
    ...
    async fn start_from_external_url(base_url: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // PostgreSQL does not accept a bound parameter for a database identifier.
    ...
    fn drop_owned_database(&mut self) {
        ...
        // Drop runs from a synchronous destructor. Use a short-lived runtime on another
        // thread so cleanup does not attempt to block the active asynchronous test executor.
...
async fn drop_isolated_postgres_database(
    ...
    // Terminate leftover fixture connections so DROP DATABASE is not blocked.
