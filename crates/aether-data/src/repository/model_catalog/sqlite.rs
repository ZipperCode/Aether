...
SELECT gm.id AS global_model_id, gm.name AS global_model_name,
       ...
       COALESCE(m.is_available, 1) AS provider_model_is_available,
...
m.is_active AS provider_model_is_active, COALESCE(m.is_available, 1) AS provider_model_is_available,
...
#[derive(Debug, Clone)]
pub struct SqliteModelCatalogReadRepository {
    pool: SqlitePool,
}

impl SqliteModelCatalogReadRepository {
...
#[async_trait]
impl ModelCatalogReadRepository for SqliteModelCatalogReadRepository {
