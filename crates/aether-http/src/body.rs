//! Bounded response-body collection and decoding shared by HTTP runtimes.

use flate2::read::{DeflateDecoder, GzDecoder};
use std::io::Read;

/// Stable error code returned when an HTTP response exceeds its configured limit.
pub const RESPONSE_TOO_LARGE_ERROR: &str = "response_too_large";

/// Errors raised while enforcing an HTTP response-body size limit.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub enum BoundedBodyError {
    /// The declared or observed response size exceeds the configured limit.
    #[error("response_too_large")]
    ResponseTooLarge,
    /// Reserving memory for the response body failed.
    #[error("response body allocation failed")]
    AllocationFailed,
}

/// Errors raised while reading a bounded reqwest response body.
#[derive(Debug, thiserror::Error)]
pub enum BoundedBodyReadError {
    /// The response violated the configured size or allocation bound.
    #[error(transparent)]
    Bounds(#[from] BoundedBodyError),
    /// Reading the upstream response body failed.
    #[error("response body read failed: {0}")]
    Read(#[from] reqwest::Error),
}

/// Incrementally collects response chunks without exceeding a fixed byte limit.
#[derive(Debug)]
...
impl BoundedBodyCollector {
    /// Creates a collector and rejects an oversized declared content length early.
    ...
    /// Appends one response chunk while enforcing the configured byte limit.
    ...
    /// Returns the collected response bytes.
    pub fn into_bytes(self) -> Vec<u8> {
        self.bytes
    }
}

/// Reads a reqwest response body while enforcing the supplied byte limit.
...
/// Decodes a gzip or deflate response body without allowing decompression past the limit.
///
/// Unsupported or malformed encodings return `Ok(None)` so callers can retain
/// their existing raw-body fallback behavior.
