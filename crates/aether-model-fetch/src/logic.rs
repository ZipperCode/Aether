...
/// Codex 图片目录补全使用的 API 格式。
pub const CODEX_IMAGE_API_FORMAT: &str = "openai:image";

/// 把 gpt-image-2 补全进 Codex 兼容模型目录（legacy/association 投影）。
///
/// 仅应由确认 provider_type=codex 的调用方使用；`image_endpoint_ids` 必须是提供商
/// 当前 active 且 Key.api_formats 允许 `openai:image` 的真实端点 ID，为空或全空白时
/// 零改动并返回 `false`（不扩权）。新追加的条目固定为
/// `{id, object, owned_by, display_name, api_formats:["openai:image"],
/// supports_image_generation:true, endpoint_ids:去重后的真实 IDs}`；若目录中已有
/// 同名条目（未来上游自己返回时）则按 ID 去重，不重复追加，只合并图片格式、能力
/// 标记与端点。原生 Codex 卡片与 codex_models 元数据不经过本函数，保持不污染。
pub fn supplement_codex_image_models(
    models: &mut Vec<Value>,
    image_endpoint_ids: &[String],
) -> bool {
    let mut endpoint_ids = BTreeSet::new();
    for endpoint_id in image_endpoint_ids {
    ...
    let existing = models.iter_mut().find(|model| {
        model
            .get("id")
            .and_then(Value::as_str)
            .map(str::trim)
    ...
    if let Some(model) = existing {
        ...
        let mut formats = json_string_list(object.get("api_formats"))
            .into_iter()
            .map(|format| normalize_api_format(&format))
        ...
        {
            formats.push(CODEX_IMAGE_API_FORMAT.to_string());
        }
        object.insert(
            "api_formats".to_string(),
            Value::Array(formats.into_iter().map(Value::String).collect()),
        );
        object.insert("supports_image_generation".to_string(), Value::Bool(true));
        let mut merged_endpoint_ids = json_string_list(object.get("endpoint_ids"))
            .into_iter()
            .collect::<BTreeSet<_>>();
        merged_endpoint_ids.extend(endpoint_ids);
        object.insert(
            "endpoint_ids".to_string(),
            Value::Array(merged_endpoint_ids.into_iter().map(Value::String).collect()),
        );
        return true;
    }

    models.push(json!({
        "id": aether_ai_formats::api::CODEX_OPENAI_IMAGE_DEFAULT_MODEL,
        "object": "model",
        "owned_by": "openai",
        "display_name": "GPT Image 2",
        "api_formats": [CODEX_IMAGE_API_FORMAT],
        "supports_image_generation": true,
        "endpoint_ids": endpoint_ids.into_iter().collect::<Vec<_>>(),
    }));
    true
}
