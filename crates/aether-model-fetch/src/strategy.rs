...
use crate::logic::{
    aggregate_models_for_cache, codex_model_identity, extract_error_message, json_string_list,
...
async fn fetch_standard_models(
    ...
    let mut all_legacy_models = Vec::new();
    ...
    for transport in transports {
        match fetch_standard_models_for_transport(runtime, transport, codex_client_version).await {
            Ok(outcome) => {
                let mut endpoint_legacy_models = if is_codex {
                    project_codex_models_for_legacy_cache([(
                        transport.endpoint.api_format.as_str(),
                        outcome.cached_models.as_slice(),
                    )])
                } else {
                    outcome.legacy_models.clone()
                };
                if outcome.has_success {
                    // 动态 Codex 卡片保持原样；Endpoint 绑定仅写入兼容目录投影。
                    attach_fallback_endpoint_id(&mut endpoint_legacy_models, transport);
                }
                all_legacy_models.extend(endpoint_legacy_models);
    ...
    outcome.legacy_models = aggregate_models_for_cache(&all_legacy_models);
    Ok(outcome
        .with_errors(errors)
...
async fn fetch_standard_models_for_transport(
    ...
    for _ in 0..20 {
        ...
        if is_codex {
            ...
            for mut model in parsed.cached_models {
