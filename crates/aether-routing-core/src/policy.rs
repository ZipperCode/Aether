...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
/// 针对一次请求解析完成的路由策略，包含排序、重试、覆盖和变更计划。
...
/// 将模型级允许列表及 Provider、Key、Pool 覆盖叠加到已解析策略。
...
/// 应用一条匹配规则动作，包括粘性预算和可选格式级 Key 优先级。
fn apply_action(
