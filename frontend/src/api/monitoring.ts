...
const CAPACITY_STAGES: Array<{ stage: string; label: string }> = [
  ...
  { stage: 'candidate_database_read', label: '候选数据库读取' },
  { stage: 'candidate_global_model_resolve', label: '全局模型解析' },
  { stage: 'candidate_mapping_match', label: '模型映射匹配' },
  { stage: 'candidate_enumeration', label: '候选枚举' },
  { stage: 'candidate_dynamic_eligibility', label: '动态资格筛选' },
