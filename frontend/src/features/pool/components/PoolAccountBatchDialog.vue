...
const SNAPSHOT_BACKED_ACTIONS = new Set<PoolBatchActionValue>([
  'delete',
  'enable',
  'disable',
  'clear_proxy',
...
async function resolveSelectedItems(): Promise<PoolKeySelectionItem[]> {
  ...
  if (selectAllFiltered.value) {
    const snapshot = props.selectionSnapshot
    if (!snapshot) return []
...
async function executeAction(actionOverride?: PoolBatchActionValue): Promise<void> {
  ...
  try {
    const snapshot = selectAllFiltered.value ? props.selectionSnapshot : null
    let usedSnapshotBatchAction = false
    let selectedKeys: PoolKeySelectionItem[] = []

    if (snapshot && SNAPSHOT_BACKED_ACTIONS.has(selectedAction.value)) {
      usedSnapshotBatchAction = true
      ...
      const result = await batchActionPoolKeys(props.providerId, {
        ...
        action: selectedAction.value as 'enable' | 'disable' | 'delete' | 'clear_proxy' | 'set_proxy' | 'update_settings',
        ...(payload ? { payload } : {}),
      })
      ...
      selectedKeys = await resolveSelectedItems()
    ...
    if (usedSnapshotBatchAction) {
      // 快照批量接口已在上方完成，不再将全量账号展开为 ID 分批执行。
    } else if (selectedAction.value === 'refresh_quota') {
