...
const isDeviceBrowserProvider = computed(() => isKiroProvider.value || isWindsurfProvider.value || isNousProvider.value || isXaiProvider.value)
...
const importManualPlaceholder = computed(() => {
  ...
  if (isNousProvider.value) {
    return legacyT('粘贴 Hermes OAuth JSON，或 Refresh Token')
  }
...
async function startDeviceAuth() {
  ...
  try {
    ...
    const resp = await startDeviceAuthorize(props.providerId, {
      ...
      start_url: (isWindsurf || isXai || isNousProvider.value) ? undefined : (isBuilderID ? BUILDER_ID_START_URL : (isSocial ? undefined : (device.value.start_url.trim() || undefined))),
      region: (isWindsurf || isXai || isNousProvider.value) ? undefined : (isBuilderID || isSocial ? BUILDER_ID_REGION : (device.value.region.trim() || undefined)),
      proxy_node_id: selectedProxyNodeId.value || undefined,
    })
