...
function getQuotaSnapshotForProvider(
  key: EndpointAPIKey,
  providerType: 'codex' | 'kiro' | 'windsurf' | 'antigravity' | 'chatgpt_web' | 'gemini_cli' | 'grok' | 'xai' | 'nous',
...
async function autoRefreshQuotaInBackground(): Promise<boolean> {
  ...
  const providerType = provider.value?.provider_type
  if (providerType !== 'codex' && providerType !== 'gemini_cli' && providerType !== 'antigravity' && providerType !== 'kiro' && providerType !== 'windsurf' && providerType !== 'chatgpt_web' && providerType !== 'grok' && providerType !== 'xai' && providerType !== 'nous' && !isOfficialQuotaProviderType(providerType)) return false
