...
describe('scheduling provider filtering', () => {
  // 多模型并集沿用单模型排序的资格约束，停用或没有启用 Key 的提供商不可排序。
  it('keeps inactive providers and providers without active keys out of selected-model ordering', async () => {
    const items = [
      ...providerSources,
      { ...providerSources[0], id: 'inactive', name: '停用提供商', is_active: false },
      { ...providerSources[1], id: 'no-key', name: '无启用 Key 提供商', active_keys: 0 },
    ]
    vi.mocked(getProvidersSummary).mockResolvedValue({
      items, total: items.length, page: 1, page_size: 9999,
    })
    const { root } = mountEditor(['model-a', 'model-b'])
    await vi.waitFor(() => expect(providerNames(root)).toEqual(['提供商 A', '提供商 B', '共享提供商']))
    await clickButton(root, '全部模型')
    await vi.waitFor(() => expect(providerNames(root)).toHaveLength(items.length))
  })
  ...
  // 多模型排序临时隐藏的关联项仍保留原有覆盖值，再次选回模型时无需重配。
  it('retains hidden provider priorities when reordering a narrower model selection', async () => {
    const { root, config } = mountEditor(['model-a', 'model-b'])
    await vi.waitFor(() => expect(providerNames(root)).toHaveLength(3))
    const row = [...root.querySelectorAll<HTMLElement>('[draggable="true"]')]
      .find(element => element.textContent?.includes('提供商 A'))!
    const input = row.querySelector<HTMLInputElement>('input[type="number"]')!
    input.value = '77'
    input.dispatchEvent(new Event('change', { bubbles: true }))
    await nextTick()
    ...
    await toggleModel(root, 'model-a')
    const remainingRows = root.querySelectorAll<HTMLElement>('[draggable="true"]')
    remainingRows[0].dispatchEvent(new Event('dragstart', { bubbles: true }))
    remainingRows[1].dispatchEvent(new Event('drop', { bubbles: true }))
    await nextTick()
    expect(getModelPolicy(config.value, 'model-b').provider_priority_overrides['provider-a']).toBe(77)

    await toggleModel(root, 'model-a')
    for (const model of ['model-a', 'model-b']) {
      expect(getModelPolicy(config.value, model).provider_priority_overrides['provider-a']).toBe(77)
    }
  })
