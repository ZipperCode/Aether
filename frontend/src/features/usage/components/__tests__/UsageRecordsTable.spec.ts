...
describe('UsageRecordsTable', () => {
  ...
  it('shows generic skipped-candidate markers to users on desktop and mobile', () => {
    const root = mountUsageRecordsTable([buildRecord({
      has_skipped_candidate: true,
      has_fallback: true,
      skipped_candidate_reasons: ['key_rpm_exhausted'],
    ...
    const markers = root.querySelectorAll('[data-usage-attempt-marker="skipped-candidate"]')
    expect(markers).toHaveLength(2)
    for (const marker of markers) {
      expect(marker.getAttribute('title')).toContain('调度阶段被跳过')
      expect(marker.getAttribute('title')).not.toContain('密钥')
      expect(marker.getAttribute('aria-label')).toBe('有候选被调度跳过')
    }
    expect(root.querySelector('[data-usage-attempt-marker="fallback"]')).toBeNull()
  })
