...
export const CANDIDATE_SKIP_REASON_LABELS: Record<string, string> = {
  ...
  // 持久化额度停用需人工恢复，不能与余额不足或读取故障混为一谈。
  key_quota_exhausted: '密钥额度耗尽，需管理员手动恢复',
  key_balance_below_minimum: '密钥可用余额不足调度要求',
  ...
  pool_key_quota_exhausted: '号池密钥额度耗尽，需管理员手动恢复',
  pool_balance_below_minimum: '号池账号可用余额不足调度要求',
  pool_key_state_unavailable: '无法读取号池密钥状态',
  ...
  unclassified_skip: '未归类的调度原因',
}

/** 后端无法归类时使用的占位原因。 */
export const UNCLASSIFIED_CANDIDATE_SKIP_REASON = 'unclassified_skip'
