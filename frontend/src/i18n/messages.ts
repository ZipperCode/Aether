...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '配额详情': 'Quota details',
  '账号配额': 'Account quota',
  '标准余额': 'Standard balance',
  '标准余额不足': 'Standard balance insufficient',
  '刷新额度': 'Refresh quota',
  '额度已刷新': 'Quota refreshed',
  '额度刷新失败': 'Quota refresh failed',
  '模型调用已验证可用': 'Model call verified available',
  '模型调用验证失败': 'Model call verification failed',
  '额度查询失败，额度未知': 'Quota query failed; quota is unknown',
  '额度未知，继续参与模型调度': 'Quota unknown; model scheduling remains enabled',
  '模型配额': 'Model quota',
  '周限额': 'Weekly limit',
  '月限额': 'Monthly limit',
  '5H限额': '5H limit',
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^确定删除选中的 (\d+) 个模型吗？\n\n此操作不可撤销。$/u, match => `Delete the selected ${match[1]} ${match[1] === '1' ? 'model' : 'models'}?\n\nThis cannot be undone.`],
  [/^成功删除 (\d+) 个模型$/u, match => `Deleted ${match[1]} ${match[1] === '1' ? 'model' : 'models'}`],
  [/^(\d+) 个模型删除失败$/u, match => `Failed to delete ${match[1]} ${match[1] === '1' ? 'model' : 'models'}`],
