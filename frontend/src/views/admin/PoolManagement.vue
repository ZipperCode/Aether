...
let keysRequestId = 0
let selectionSnapshotRequestId = 0
...
const creatingPoolKeySelectionSnapshot = ref(false)
const poolKeySelectionBusy = computed(() => (
  keysLoading.value || keysSearchPending.value || creatingPoolKeySelectionSnapshot.value
))
...
const selectedPoolKeySnapshot = ref<PoolSelectionSnapshot | null>(null)
const {
  ...
  resetSelection: resetPoolKeySelectionState,
...
function normalizePoolSelectionSnapshot(
  ...
  if (!snapshot || !id || !Number.isFinite(total) || total < 0) return null
  return { ...snapshot, id, total }
}

async function toggleAllFilteredPoolKeys(): Promise<void> {
  if (poolKeySelectionBusy.value || keyPage.value.total === 0) return
  if (isAllFilteredPoolKeysSelected.value) {
  ...
  const requestId = ++selectionSnapshotRequestId
  const expectedTotal = keyPage.value.total
  creatingPoolKeySelectionSnapshot.value = true
  try {
    const search = searchQuery.value.trim()
    const result = await createPoolKeySelectionSnapshot(providerId, {
      ...
      expected_page_key_ids: keyPage.value.keys.map(key => key.key_id),
    })
    if (requestId !== selectionSnapshotRequestId || selectedProviderId.value !== providerId) return

    const snapshot = normalizePoolSelectionSnapshot(result.selection_snapshot)
    if (!snapshot) {
      showWarning('当前筛选快照不可用，请刷新列表后重试')
      return
    }
    ...
    } else if (result.selection_snapshot_mismatch || snapshot.total !== expectedTotal) {
      keyPage.value = { ...keyPage.value, total: snapshot.total }
    }

    selectedPoolKeySnapshot.value = snapshot
    toggleSelectFilteredPoolKeys(true)
...
/** Pool 页面内部额度行，保留真实窗口时间与可选家族范围。 */
interface QuotaProgressItem {
  label: string
  remainingPercent: number
  /** 仅用于兼容紧凑数字布局；最终 Antigravity 家族摘要使用普通进度条。 */
  numericOnly?: boolean
  sortOrder?: number
  detail?: string
...
/** 为当前页每个 Key 解析 Provider 专用额度行，供桌面与移动端共用。 */
...
/** 将每个 Key 的业务额度行转换为稳定的文字、颜色和布局属性。 */
...
/** 在单 Key 启停成功后同步当前页和 Provider 汇总计数。 */
...
/** 把批量额度刷新结果增量写回当前页，不重载并丢失本地 UI 状态。 */
...
/** 优先恢复服务端 reservation，其次读取同凭据代际下的本地幂等键。 */
...
/** 校验 Provider、凭据代际、可用机会和全页互斥状态后决定是否允许消费。 */
...
/**
 * 以凭据代际绑定的幂等键消费一次 Codex 重置机会，并把返回快照合并到当前页。
 * 非终态保留幂等键供重试；凭据已变化时清除旧键，避免跨账号重放。
 */
...
/** 将 Pool 列表鉴权类型归一为编辑表单支持的枚举，不改动已存凭据内容。 */
...
/** 将 Pool Key 快照转换为编辑表单模型，并保留可空并发上限。 */
...
/**
 * 将统一额度快照投影为 Provider 专用展示行。
 * Antigravity 复用共享家族摘要，并让最低剩余窗口携带真实重置倒计时。
 */
