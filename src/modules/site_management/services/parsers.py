"""Parsing utilities for WebDav backup payloads."""
...
def parse_all_api_hub_accounts(raw: dict[str, Any]) -> list[ImportedAccount]:
    ...
    cookie_candidates = ("sessionCookie", "cookie", "cookie_string", "cookieString", "cookieValue")
    ...
    for account in _extract_accounts_list(raw):
        ...
        result.append(ImportedAccount(
            domain=domain, session_cookie=session_cookie, site_url=site_url,
            auth_type=auth_type, user_id=user_id, cookie_value=cookie_value or None,
