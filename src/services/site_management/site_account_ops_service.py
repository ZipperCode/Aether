...
class SiteAccountOpsService:
    ...
    def _build_target(self, account: SiteAccount) -> OpsExecutionTarget | None:
        provider_config = self._extract_provider_ops_config(account.provider_id)
        ...
        architecture_id = str(
            ...
            or provider_config.get("architecture_id")
        ...
        connector_config = self._resolve_connector_config(account_config, provider_config)
        actions = self._resolve_actions(account_config, provider_config)
        credentials = self._resolve_credentials(account, provider_config)
    ...
    def _extract_provider_ops_config(self, provider_id: str | None) -> dict[str, Any]:
        if not provider_id:
            return {}
        provider = self.db.query(Provider).filter(Provider.id == provider_id).first()
        if not provider or not isinstance(provider.config, dict):
            return {}
        provider_ops = provider.config.get("provider_ops")
        return provider_ops if isinstance(provider_ops, dict) else {}
    ...
    def _resolve_connector_config(
        ...
        provider_connector = provider_config.get("connector", {})
        if isinstance(provider_connector, dict):
            provider_connector_config = provider_connector.get("config")
            if isinstance(provider_connector_config, dict):
                return dict(provider_connector_config)
        return {}
    ...
    def _resolve_actions(
        ...
        provider_actions = provider_config.get("actions")
        if isinstance(provider_actions, dict):
            return dict(provider_actions)
        return {}
    ...
    def _resolve_credentials(
        ...
        provider_connector = provider_config.get("connector")
        if isinstance(provider_connector, dict):
            provider_credentials = provider_connector.get("credentials")
            if isinstance(provider_credentials, dict):
                return self._decrypt_provider_credentials(provider_credentials)
        return {}
    ...
    def _to_checkin_result_from_new_api(result: ActionResult) -> ActionResult:
        ...
        elif SiteAccountOpsService._contains_checkin_not_supported_signal(message):
    ...
    def _contains_checkin_not_supported_signal(message: str | None) -> bool:
        ...
        if not text:
            return False
        return any(ind in text for ind in SiteAccountOpsService._CHECKIN_NOT_SUPPORTED_INDICATORS)
    ...
    def _resolve_auth_type(
        *,
        ...
        if auth_type_value is None:
            provider_connector = provider_config.get("connector")
            if isinstance(provider_connector, dict):
                auth_type_value = provider_connector.get("auth_type")
        if auth_type_value is None:
